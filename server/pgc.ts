...
import { normalizeAccountingRuleOperation } from "./accounting-rule-operations";
...
export async function importPendingPgcAccountsForUser(input: { userId: number; organizationId: number; versionId: number; accounts: PgcAccountDraft[] }) {
  const db = await getDb(); if (!db) throw new Error("Database unavailable");
  ...
  for (const account of input.accounts) {
    ...
    const source = await db.select({ id: pgcSources.id, verificationStatus: pgcSources.verificationStatus }).from(pgcSources).where(and(eq(pgcSources.id, account.sourceId), eq(pgcSources.organizationId, input.organizationId), eq(pgcSources.versionId, input.versionId))).limit(1);
    if (!source[0] || source[0].verificationStatus !== "CONFIRMED") throw new Error("PGC_PENDING_IMPORT_SOURCE_NOT_CONFIRMED");
...
export async function createAccountingRuleForUser(input: { userId: number; organizationId: number; versionId: number; companyId?: number; operation: string; documentType?: string; debitAccountId: number; creditAccountId: number; ivaAccountId?: number; nature?: string; taxType?: "NONE" | "IVA" | "IRT"; calculationBase?: "NONE" | "NET" | "GROSS" | "WITHHOLDING_BASE" | "FIXED"; taxRate?: number | null; costCenterCode?: string; priority?: number; effectiveFrom: Date; effectiveTo?: Date | null; sourceId?: number; notes?: string }) {
  ...
  const source = await db.select({ id: pgcSources.id, verificationStatus: pgcSources.verificationStatus }).from(pgcSources).where(and(eq(pgcSources.id, input.sourceId as number), eq(pgcSources.organizationId, input.organizationId), eq(pgcSources.versionId, input.versionId))).limit(1);
  if (!source[0] || source[0].verificationStatus !== "CONFIRMED") throw new Error("PGC_RULE_SOURCE_NOT_CONFIRMED");
  const ids = [input.debitAccountId, input.creditAccountId, ...(input.ivaAccountId ? [input.ivaAccountId] : [])];
  const accounts = await db.select({ id: pgcAccounts.id, organizationId: pgcAccounts.organizationId, versionId: pgcAccounts.versionId, acceptsEntries: pgcAccounts.acceptsEntries, validationStatus: pgcAccounts.validationStatus, nature: pgcAccounts.nature, iva: pgcAccounts.iva }).from(pgcAccounts).where(inArray(pgcAccounts.id, ids));
  ...
  const debit = accounts.find((account) => account.id === input.debitAccountId);
  const credit = accounts.find((account) => account.id === input.creditAccountId);
