...
import { businessDocuments, companies, documentImportBatches, documentImportRows, fileAssets, products } from "../drizzle/schema";
...
describe("practical import and review workflow", () => {
  it("executes valid/invalid CSV and XLSX flows, confirmation and tenant isolation", async () => {
    ...
    try {
      ...
      const documentCsv = `documentNumber,documentType,currency,ivaRegime,netAmount,taxAmount,totalAmount,lines\nFT TEST/1,FT,AOA,EXCLUSAO,100,0,100,"[{""description"":""Serviço"",""quantity"":1,""unitPrice"":100,""netAmount"":100,""taxAmount"":0,""totalAmount"":100}]"`;
      const documentBatch = await caller.exports.createReviewBatch({ organizationId: TEST_ORGANIZATION_ID, companyId: TEST_COMPANY_ID, kind: "documents", filename: "documentos-validos.csv", dataBase64: Buffer.from(documentCsv).toString("base64") });
      ...
      const existingDocument = await db!.select({ id: businessDocuments.id, documentNumber: businessDocuments.documentNumber }).from(businessDocuments).where(eq(businessDocuments.companyId, TEST_COMPANY_ID)).limit(1);
      if (existingDocument[0]) {
        const rendered = await caller.documents.renderPreparationPdf({ companyId: TEST_COMPANY_ID, documentId: existingDocument[0].id });
        ...
        expect(stored[0]).toMatchObject({ mimeType: "application/pdf", filename: `${existingDocument[0].documentNumber}.pdf` });
