...
describe("Repair Lubatec router integration", () => {
  ...
  it("reads every relevant tenant-aware surface without creating transactions", async () => {
    ...
    expect(journal.entries).toEqual(expect.arrayContaining([expect.objectContaining({ accountCode: "45.1.1", accountName: "Caixa Repair Lubatec", debit: 50000, credit: 0 }), expect.objectContaining({ accountCode: "61.3.1", accountName: "Mercadorias — Mercado nacional", debit: 0, credit: 50000 })]));
    const ledger = await caller.reports.ledger({ companyId: 1 });
    expect(ledger.entries).toEqual(expect.arrayContaining([expect.objectContaining({ accountCode: "45.1.1", debit: 50000, credit: 0, balance: 50000 }), expect.objectContaining({ accountCode: "61.3.1", debit: 0, credit: 50000, balance: 0 })]));
