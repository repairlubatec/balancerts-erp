...
export type AuditMetricRow = { key: string; label: string; count: number };
...
export function aggregateAuditMetrics(events: AuditMetricEvent[], labels: Record<string, string> = {}) {
  ...
  const actionRows = Array.from(actions, ([key, count]) => ({ key, label: labels[key] ?? key, count })).sort((a, b) => b.count - a.count || a.label.localeCompare(b.label, "pt-PT")).slice(0, 5);
  const userRows = Array.from(users, ([key, value]) => ({ key, label: value.label, count: value.count })).sort((a, b) => b.count - a.count || a.label.localeCompare(b.label, "pt-PT")).slice(0, 5);
