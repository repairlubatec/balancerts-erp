      ...
      <Card className="rounded-sm border-[#bfc9d4] bg-[#fbfcfd] shadow-none">
        ...
        <CardContent className="space-y-4">
          {!selectedStudyId ? <p className="text-xs text-slate-500">Crie ou seleccione um estudo para iniciar a análise financeira.</p> : <>
            ...
            {scenarios.data?.length ? <div className="space-y-2"><p className="text-xs font-semibold text-[#102a43]">Cenários guardados</p>{scenarios.data.map((scenario) => <div key={scenario.id} className="flex items-center justify-between rounded border border-[#dbe5f1] bg-white px-3 py-2 text-xs"><div><span>{scenario.name}</span><p className="text-[10px] text-slate-500">Capital próprio: {Number(scenario.equityAmount ?? 0).toLocaleString("pt-PT", { maximumFractionDigits: 0 })} · Dívida: {Number(scenario.debtAmount ?? 0).toLocaleString("pt-PT", { maximumFractionDigits: 0 })}</p></div><span className={scenario.decision === "PROSSEGUIR" ? "font-semibold text-emerald-700" : scenario.decision === "REJEITAR" ? "font-semibold text-rose-700" : "font-semibold text-amber-700"}>{scenario.decision === "PROSSEGUIR" ? "Prosseguir" : scenario.decision === "REJEITAR" ? "Rejeitar" : "Rever"}</span></div>)}</div> : null}
