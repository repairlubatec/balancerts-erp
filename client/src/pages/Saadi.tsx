...
export default function Saadi() {
  ...
  const snapshots = trpc.saadi.snapshots.useQuery(activeCompanyId && organizationId ? { companyId: activeCompanyId, organizationId, studyId: selectedStudyId } : { companyId: 0, organizationId: 0, studyId: 0 }, { enabled: Boolean(activeCompanyId && organizationId) });
  const versions = trpc.saadi.versions.useQuery(activeCompanyId && organizationId && selectedStudyId ? { companyId: activeCompanyId, organizationId, studyId: selectedStudyId } : { companyId: 0, organizationId: 0, studyId: 0 }, { enabled: Boolean(activeCompanyId && organizationId && selectedStudyId) });
  ...
  return <div className="space-y-4 p-4 text-[#1d2a38]">
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
      <Card className="rounded-sm border-[#bfc9d4] bg-[#fbfcfd] shadow-none"><CardHeader className="pb-2"><CardTitle className="text-sm text-[#102a43]">Novo estudo</CardTitle></CardHeader><CardContent><form className="space-y-2" onSubmit={(event) => { event.preventDefault(); if (!activeCompanyId || !organizationId || !code.trim() || !name.trim()) return; createStudy.mutate({ companyId: activeCompanyId, organizationId, studyCode: code, name, baseCurrency: selected?.company.functionalCurrency ?? "AOA" }); }}><Input aria-label="Código do estudo" placeholder="Código, por exemplo INV-001" value={code} onChange={(event) => setCode(event.target.value)} /><Input aria-label="Nome do estudo" placeholder="Nome do estudo" value={name} onChange={(event) => setName(event.target.value)} /><Button type="submit" disabled={!activeCompanyId || !code.trim() || !name.trim() || createStudy.isPending} className="w-full bg-[#1267d6]"><Plus className="mr-1 h-4 w-4" />{createStudy.isPending ? "A registar…" : "Criar estudo"}</Button>{createStudy.error && <p role="alert" className="text-xs text-rose-700">Não foi possível criar o estudo. Verifique os dados e as permissões da empresa.</p>}{createStudy.isSuccess && <p role="status" className="text-xs text-emerald-700">Estudo criado e pronto para configuração.</p>}</form></CardContent></Card>
      <Card className="rounded-sm border-[#bfc9d4] bg-[#fbfcfd] shadow-none"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm text-[#102a43]">Estudos da empresa</CardTitle><Button type="button" size="sm" variant="outline" onClick={() => studies.refetch()} disabled={studies.isFetching}><RefreshCw className="mr-1 h-3.5 w-3.5" /> Actualizar</Button></CardHeader><CardContent>{studies.isLoading ? <p className="text-xs text-slate-500">A carregar estudos…</p> : studies.error ? <p className="text-xs text-rose-700">Não foi possível carregar os estudos. Verifique as permissões da empresa.</p> : !activeCompanyId ? <p className="text-xs text-slate-500">Seleccione uma empresa para consultar o seu contexto SAADI.</p> : !(studies.data ?? []).length ? <p className="text-xs text-slate-500">Ainda não existem estudos nesta empresa.</p> : <div className="space-y-2">{(studies.data ?? []).map((study) => <button type="button" key={study.id} onClick={() => setStudyId(study.id)} className={`flex w-full items-center justify-between rounded border px-3 py-2 text-left ${selectedStudyId === study.id ? "border-[#1267d6] bg-[#eef5ff]" : "border-[#dbe5f1] bg-white"}`}><div><p className="text-xs font-semibold text-[#102a43]">{study.name}</p><p className="text-[11px] text-slate-500">{study.studyCode} · {study.baseCurrency}</p></div><Badge variant="outline">{study.status === "DRAFT" ? "Rascunho" : study.status === "ACTIVE" ? "Activo" : "Arquivado"}</Badge></button>)}</div>}</CardContent></Card>
    </div>
    <div className="grid gap-4 lg:grid-cols-2"><Card className="rounded-sm border-[#bfc9d4] bg-[#fbfcfd] shadow-none"><CardHeader className="pb-2"><CardTitle className="text-sm text-[#102a43]">Capturas de dados</CardTitle></CardHeader><CardContent>{snapshots.isLoading ? <p className="text-xs text-slate-500">A carregar capturas…</p> : !(snapshots.data ?? []).length ? <p className="text-xs text-slate-500">Sem capturas para o estudo seleccionado.</p> : <div className="space-y-2">{(snapshots.data ?? []).map((snapshot) => <div key={snapshot.id} className="rounded border border-[#dbe5f1] bg-white p-2"><div className="flex justify-between text-xs"><span className="font-semibold text-[#102a43]">{snapshot.status === "READY" ? "Pronto" : snapshot.status === "STALE" ? "Desactualizado" : "Inválido"}</span><span className="text-slate-500">{snapshot.sourceFingerprint.slice(0, 12)}…</span></div><p className="mt-1 text-[11px] text-slate-500">Chave de repetição segura: {snapshot.idempotencyKey}</p></div>)}</div>}</CardContent></Card><Card className="rounded-sm border-[#bfc9d4] bg-[#fbfcfd] shadow-none"><CardHeader className="pb-2"><CardTitle className="text-sm text-[#102a43]">Versões e origem dos dados</CardTitle></CardHeader><CardContent>{versions.isLoading ? <p className="text-xs text-slate-500">A carregar versões…</p> : !(versions.data ?? []).length ? <p className="text-xs text-slate-500">Sem versões criadas para o estudo seleccionado.</p> : <div className="space-y-2">{(versions.data ?? []).map((version) => <div key={version.id} className="rounded border border-[#dbe5f1] bg-white p-2"><div className="flex justify-between text-xs"><span className="font-semibold text-[#102a43]">Versão {version.versionNumber}</span><Badge variant="outline">{version.status === "APPROVED" ? "Aprovada" : version.status === "IN_REVIEW" ? "Em revisão" : version.status === "ARCHIVED" ? "Arquivada" : "Rascunho"}</Badge></div><p className="mt-1 text-[11px] text-slate-500">Integridade: {version.versionHash.slice(0, 16)}… · Criada por {version.createdBy}</p></div>)}</div>}</CardContent></Card></div>
  </div>;
}
