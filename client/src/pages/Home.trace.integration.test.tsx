...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    companies: { list: { useQuery: () => ({ data: [{ company: { id: 1, name: "Repair Lubatec", nif: "5001121871", configurationStatus: "READY", ivaRegime: "EXCLUSAO", functionalCurrency: "AOA", organizationId: 1 } }], isLoading: false }) }, create: { useMutation: (options?: { onSuccess?: (result: { company: { name: string } }) => unknown; onError?: (error: Error) => unknown }) => ({ mutate: vi.fn((input: unknown) => options?.onSuccess?.({ company: { name: (input as { name: string }).name } })), isPending: false, error: null }) }, periods: { useQuery: () => ({ data: [{ period: { id: 1, year: 2026, month: 1, status: "OPEN" } }], isLoading: false }) }, memberships: { useQuery: () => ({ data: [], isLoading: false, refetch: vi.fn() }) }, addMembership: { useMutation: (options?: { onSuccess?: () => unknown; onError?: (error: Error) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, updateMembership: { useMutation: (options?: { onSuccess?: () => unknown; onError?: (error: Error) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) } },
    audit: { list: { useQuery: () => ({ data: locationState.auditEvents, isLoading: false }) }, exportPgcAlertsPdf: { useMutation: (options?: { onSuccess?: (result: { filename: string; mimeType: string; dataBase64: string; eventCount: number }) => unknown; onError?: (error: Error) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.({ filename: "alertas.pdf", mimeType: "application/pdf", dataBase64: "", eventCount: 0 })), isPending: false, error: null }) } },
    ...
    inventory: { balances: { useQuery: () => ({ data: [], isLoading: false }) }, warehouses: { useQuery: () => ({ data: [], isLoading: false }) }, counts: { useQuery: () => ({ data: [], isLoading: false, refetch: vi.fn() }) }, reconcile: { useQuery: () => ({ data: undefined, isLoading: false, isFetching: false, error: null }) }, record: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, transfer: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, createWarehouse: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, createCount: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, reviewCount: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, applyCount: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) } },
    documents: { list: { useQuery: () => ({ data: [], isLoading: false }) }, seriesList: { useQuery: () => ({ data: [{ series: { id: 1, code: "FT", documentType: "FT", nextNumber: 1 } }], isLoading: false }) }, createSeries: { useMutation: (options?: { onSuccess?: () => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.()), isPending: false, error: null }) }, createDraft: { useMutation: (options?: { onSuccess?: (result: { documentNumber: string }) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.({ documentNumber: "FT 2026/TEST" })), isPending: false, error: null }) }, transition: { useMutation: (options?: { onSuccess?: (result: { from: "DRAFT"; to: "VALIDATED" }) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.({ from: "DRAFT", to: "VALIDATED" })), isPending: false, error: null }) } },
    accounting: { accounts: { useQuery: () => ({ data: [{ account: { id: 11, code: "11", name: "Meios monetários", postable: 1 } }, { account: { id: 21, code: "21", name: "Clientes", postable: 1 } }], isLoading: false }) }, costCenters: { useQuery: () => ({ data: [], isLoading: false }) }, validateEntry: { useMutation: (options?: { onSuccess?: (result: { ok: true; debit: number; credit: number }) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.({ ok: true, debit: 100, credit: 100 })), isPending: false, error: null }) }, post: { useMutation: (options?: { onSuccess?: (result: { entryId: number }) => unknown }) => ({ mutate: vi.fn(() => options?.onSuccess?.({ entryId: 1 })), isPending: false, error: null }) } },
    closing: {
      readiness: { useQuery: () => ({ data: { periodId: 1, companyId: 1, status: "OPEN", canClose: false, blockers: [{ code: "DOCUMENTS_VALIDATED", label: "Documentos do período validados", passed: false, blocking: true }], completed: 0, total: 5, checks: [{ code: "DOCUMENTS_VALIDATED", label: "Documentos do período validados", passed: false, blocking: true }, { code: "POSTINGS_RECONCILED", label: "Lançamentos reconciliados com o razão", passed: false, blocking: true }, { code: "TAX_REGISTER_REVIEWED", label: "Registo fiscal revisto", passed: false, blocking: true }, { code: "BANK_RECONCILED", label: "Caixa e bancos reconciliados", passed: false, blocking: false }, { code: "AUDIT_COMPLETE", label: "Pendências de auditoria revistas", passed: false, blocking: false }] }, isLoading: false, isFetching: false, refetch: vi.fn(async () => ({ data: { periodId: 1, companyId: 1, status: "OPEN", canClose: false, blockers: [{ code: "DOCUMENTS_VALIDATED", label: "Documentos do período validados", passed: false, blocking: true }], completed: 0, total: 5, checks: [] } })) }) },
    ...
    reports: {
      ...
      financialDashboard: { useQuery: () => ({ data: { currency: "AOA", kpis: { revenue: 0, expenses: 0, netIncome: 0, receivable: 0, payable: 0 }, revenueRows: [], expenseRows: [], monthlySeries: [], aging: { receivable: { byBucket: { CURRENT: 0, DAYS_1_30: 0, DAYS_31_60: 0, DAYS_61_90: 0, OVER_90: 0 } }, payable: { byBucket: { CURRENT: 0, DAYS_1_30: 0, DAYS_31_60: 0, DAYS_61_90: 0, OVER_90: 0 } } }, reconciliation: { balanced: true, debit: 0, credit: 0 }, comparisonPeriodId: null, comparison: { revenue: 0, expenses: 0, netIncome: 0 } }, isLoading: false, error: null }) }, archiveFinancialDashboardPdf: { useMutation: () => ({ mutate: vi.fn(), isPending: false, error: null }) },
    },
  },
}));
