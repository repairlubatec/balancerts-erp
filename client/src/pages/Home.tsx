...
function DocumentCreatePanel({ company }: { company?: { id: number; nif?: string } }) {
  ...
  const qrQuery = trpc.documents.generateAgtQr.useQuery({ companyId: company?.id ?? 0, issuerNif: company?.nif ?? "", documentNo: qrDocumentNo }, { enabled: Boolean(company?.id && company?.nif && qrDocumentNo.trim()) });
