...
const alerts = [
  { type: "critical", title: "3 documentos aguardam validação", meta: "Kwanza Norte Comércio, Lda. · FT 2026/00479–481", action: "Abrir FT 2026/00479", path: "/documentos?focus=factura_00482.pdf" },
  { type: "warning", title: "Reconciliação bancária pendente", meta: "Luz & Linha Consultoria, SU · BFA-889102", action: "Abrir BFA-889102", path: "/tesouraria?focus=BFA-889102" },
  { type: "info", title: "Período pronto para pré-fecho", meta: "BALANCERTS Serviços, Lda. · Novembro 2026", action: "Abrir checklist Nov/26", path: "/fecho?focus=Nov%202026" },
];
...
const moduleData: Record<string, { eyebrow: string; title: string; description: string; columns: string[]; rows: string[][] }> = {
  ...
  "/empresas": { eyebrow: "Estrutura empresarial", title: "Empresas", description: "Organizações, empresas, exercícios e perfis com contexto seguro.", columns: ["Empresa", "NIF", "Organização", "Moeda", "Exercício", "Estado"], rows: companies.map((c) => [c.name, c.nif, "BALANCERTS Group", "AOA", "2026", c.status]) },
...
function Overview() {
  ...
  const filteredCompanies = companies.filter((company) => `${company.name} ${company.nif}`.toLowerCase().includes(query.toLowerCase()));
