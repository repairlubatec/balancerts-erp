...
function ModulePage({ data, focus, entry, newAction, activeCompanyId }: { data: typeof moduleData[string]; focus: string | null; entry: string | null; newAction: string | null; activeCompanyId?: number }) {
  ...
  const { data: documentRows, isLoading: documentsLoading } = trpc.documents.list.useQuery({ companyId: reportCompanyId ?? 0 }, { enabled: Boolean(reportCompanyId && isBillingPage) });
