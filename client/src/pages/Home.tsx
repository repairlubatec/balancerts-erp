...
function Overview({ activeCompanyId }: { activeCompanyId?: number }) {
  ...
  return <DesktopOverviewPanel
    ...
    onOpenCompany={() => setLocation("/empresas")}
...
function ModulePage({ data, focus, entry, newAction, activeCompanyId }: { data: typeof moduleData[string]; focus: string | null; entry: string | null; newAction: string | null; activeCompanyId?: number }) {
  const [, setLocation] = useLocation();
  ...
  const operationRowIds = isAuditPage ? (auditPageRows ?? []).map(({ event }) => event.id) : isAccountingPage ? (journal?.entries ?? []).map((entry) => entry.entryId) : (isBillingPage || isDocumentsPage) ? (documentRows ?? []).map(({ document }) => document.id) : isCustomersPage ? (customerRows ?? []).map(({ counterparty }) => counterparty.id) : isSuppliersPage ? (supplierRows ?? []).map(({ counterparty }) => counterparty.id) : isStockPage ? (productRows ?? []).map(({ product }) => product.id) : isTreasuryPage ? (accountRows ?? []).map(({ account }) => account.id) : isFixedAssetsPage ? (fixedAssetRows ?? []).map(({ asset }) => asset.id) : [];
