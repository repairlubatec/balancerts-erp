...
export function AgtConsolePanel({ company }: { company?: { id: number } }) {
  ...
  return <Card className="border-[#b9d2ef] bg-[#f8fbff] shadow-sm">
    ...
    <CardContent className="space-y-2">
      ...
      {rows.map(({ submission, documents }) => <div key={submission.id} className="rounded-md border border-[#dbe5f1] bg-white p-3 text-xs"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-[#1267d6]">#{submission.id}</span><span className="font-medium text-[#102a43]">{submission.operation}</span><span className="rounded bg-[#eef5ff] px-2 py-0.5 font-semibold text-[#305b88]">{submission.state}</span><span className="text-slate-500">requestID: {submission.requestID ?? "—"}</span><Button type="button" variant="outline" size="sm" disabled={update.isPending || ["PROCESSING", "COMPLETED"].includes(submission.state)} onClick={() => update.mutate({ companyId: company.id, submissionId: submission.id, state: "PENDING" })} className="ml-auto h-7 border-[#1267d6] bg-white text-[#1267d6]">Reprocessar</Button></div><p className="mt-2 break-all text-slate-500">UUID: {submission.submissionUUID} · resultCode: {submission.resultCode ?? "—"}</p>{submission.lastError && <p className="mt-1 text-rose-700">{submission.lastError}</p>}{documents && <p className="mt-1 text-slate-500">Documento: {documents.documentNo} · estado {documents.documentStatus}</p>}</div>)}
