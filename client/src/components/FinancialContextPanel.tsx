...
export function FinancialContextPanel({ moduleTitle, activeCompanyId, selectedPeriodId, onCompanyChange, onPeriodChange }: FinancialContextPanelProps) {
  ...
  return <>
    ...
    <Card className="mt-2 border-[#b9d2ef] bg-[#fbfcfd] shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-[#dbe5f1] py-2"><div><CardTitle className="text-sm text-[#102a43]">Contabilidade — {selectedPeriod ? `${monthNames[selectedPeriod.month - 1]} ${selectedPeriod.year}` : `${monthNames[month - 1]} ${year}`}</CardTitle><p className="text-[11px] text-slate-500">Operações disponíveis no contexto seleccionado.</p></div><div className="flex gap-1"><Button type="button" size="sm" className="h-7 bg-[#1267d6] px-2 text-[11px]" onClick={() => { if (!selectedPeriod || selectedPeriod.status !== "OPEN") { setFeedback("Seleccione um período aberto antes de iniciar um lançamento."); return; } setOperation("entry"); }}>+ Novo lançamento</Button><Button type="button" size="sm" variant="outline" className="h-7 bg-white px-2 text-[11px]" onClick={() => { if (!selectedPeriod || selectedPeriod.status !== "OPEN") { setFeedback("Seleccione um período aberto antes de importar documentos."); return; } setOperation("import"); }}>Importar documento</Button></div></CardHeader>
