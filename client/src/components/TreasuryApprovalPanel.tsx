...
export function TreasuryApprovalPanel({ companyId }: { companyId?: number }) {
  const [references, setReferences] = React.useState<Record<number, string>>({});
  ...
  const approve = api.treasury.approvePayment?.useMutation ? api.treasury.approvePayment.useMutation({ onSuccess: async () => { await utils.treasury.payments.invalidate({ companyId: companyId ?? 0 }); setReferences({}); setFeedback("Pagamento aprovado, executado e auditado."); }, onError: (error: Error) => setFeedback(error.message) }) : { isPending: false, mutate: () => undefined };
