...
export function PgcaExternalSummaryPanel() {
  return (
    <Card className="rounded-sm border-[#bfc9d4] shadow-none" data-testid="pgca-external-summary">
      ...
      <CardContent className="grid min-w-0 gap-2 px-3 pb-3 sm:grid-cols-2 2xl:grid-cols-3">
        {pgcaExternalBlockers.map(blocker => {
          ...
          return <div key={blocker.label} className="min-w-0 rounded-sm border border-slate-200 bg-slate-50 px-2 py-1.5" title={blocker.reason} aria-label={`${blocker.label}: ${blocker.count} pendências, ${percent}%`}>
