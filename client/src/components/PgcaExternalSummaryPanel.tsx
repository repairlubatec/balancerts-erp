    ...
    <Card className="rounded-sm border-[#bfc9d4] shadow-none" data-testid="pgca-external-summary">
      ...
      <CardContent className="grid min-w-0 gap-2 px-3 pb-3 sm:grid-cols-2 2xl:grid-cols-3">
        {pgcaExternalBlockers.map(blocker => {
          ...
          return <button type="button" key={blocker.label} className={`min-w-0 rounded-sm border px-2 py-1.5 text-left transition-colors ${selectedLabel === blocker.label ? "border-[#1267d6] bg-blue-50" : "border-slate-200 bg-slate-50 hover:border-[#1267d6]"}`} onClick={() => onSelectBlocker?.(blocker.label)} title={`${blocker.reason} Pré-requisito: ${blocker.prerequisite} Próxima acção: ${blocker.nextAction}`} aria-label={`${blocker.label}: ${blocker.count} pendências, ${percent}%; pré-requisito em falta: ${blocker.prerequisite}; próxima acção: ${blocker.nextAction}; clicar para filtrar contas pendentes`}>
            ...
            <div className="mt-1 flex items-start gap-1 text-[9px] text-slate-500"><Info className="mt-0.5 h-2.5 w-2.5 shrink-0" aria-hidden="true" /><span><strong>Em espera:</strong> {blocker.prerequisite}</span></div><div className="mt-1 text-[9px] text-blue-700">Próxima acção: {blocker.nextAction}</div>
          </button>;
        })}
      </CardContent>
    </Card>
