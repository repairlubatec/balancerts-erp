...
function DashboardLayoutContent({
  ...
  const activeWindowState = windowStates[activePath] ?? { minimized: false, maximized: false };
  const toggleWindowMinimized = (path: string) => setWindowStates((current) => ({ ...current, [path]: { ...(current[path] ?? { minimized: false, maximized: false }), minimized: !(current[path]?.minimized ?? false) } }));
  const toggleWindowMaximized = (path: string) => setWindowStates((current) => ({ ...current, [path]: { ...(current[path] ?? { minimized: false, maximized: false }), maximized: !(current[path]?.maximized ?? false), minimized: false } }));
  ...
  const handleMenuCommand = (command: string) => {
    ...
    if (command === "shell-minimize") {
      toggleWindowMinimized(activePath);
      return;
    }
    if (command === "shell-maximize") {
      toggleWindowMaximized(activePath);
      return;
    }
      ...
      <SidebarInset className="border-l border-[#aeb8c4] bg-[#dfe4ea]">
        ...
        <main className={cn("erp-workspace flex min-h-0 flex-1 flex-col bg-[#dfe4ea] p-3", activeWindowState.maximized && "erp-workspace--maximized")}>
          <DesktopWindowFrame
            ...
            minimized={activeWindowState.minimized}
            maximized={activeWindowState.maximized}
            onMinimize={() => toggleWindowMinimized(activePath)}
            onMaximize={() => toggleWindowMaximized(activePath)}
            onClose={() => closeWorkspace(activePath)}
          >
            {children}
          </DesktopWindowFrame>
        </main>
        <DesktopWindowTaskbar
          windows={workspaceTabs.map((window) => ({ path: window.path, label: window.label, icon: window.icon, minimized: windowStates[window.path]?.minimized }))}
          activePath={activePath}
          onSelect={selectWorkspace}
        />
