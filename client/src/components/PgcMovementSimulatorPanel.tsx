...
export function PgcMovementSimulatorPanel({ organizationId, companyId, versionId, accounts }: Props) {
  ...
  const canSimulate = Boolean(organizationId && companyId && versionId && debitAccountId && creditAccountId && amount && operation.trim() && !simulate.isPending);
