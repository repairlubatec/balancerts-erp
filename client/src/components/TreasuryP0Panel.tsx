...
export function TreasuryP0Panel({ companyId, organizationId }: { companyId?: number; organizationId?: number }) {
  ...
  const statementLines = treasuryApi.treasury.statementLines?.useQuery ? treasuryApi.treasury.statementLines.useQuery({ companyId: companyId ?? 0, cashAccountId: selectedCashAccountId ? Number(selectedCashAccountId) : undefined, status: "UNMATCHED" }, { enabled: Boolean(companyId && selectedCashAccountId) }) : { data: [] as Array<{ line: { id: number; description: string; amount: string; direction: string; status: string } }> };
