...
export function DigitalArchivePanel({ companyId }: { companyId?: number }) {
  ...
  const files = trpc.files.list.useQuery({ companyId: companyId ?? 0, search: search || undefined, category: category || undefined, from: from ? new Date(`${from}T00:00:00`) : undefined, to: to ? new Date(`${to}T23:59:59`) : undefined }, { enabled: Boolean(companyId) });
  ...
  return <Card className="rounded-sm border-[#bfc9d4] bg-[#fbfcfd] shadow-none">
    ...
    <CardContent className="space-y-3 p-4">
      <div className="flex flex-wrap gap-2"><div className="relative min-w-[230px] flex-1"><Search className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Pesquisar por nome, descrição ou referência" className="h-9 bg-white pl-8 text-xs" /></div><select aria-label="Filtrar por classificação" value={category} onChange={(event) => setCategory(event.target.value as Category | "")} className="h-9 rounded-md border border-[#dbe5f1] bg-white px-2 text-xs"><option value="">Todas as classificações</option>{categories.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><Input aria-label="Data inicial do arquivo" type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="h-9 w-36 bg-white text-xs" /><Input aria-label="Data final do arquivo" type="date" value={to} onChange={(event) => setTo(event.target.value)} className="h-9 w-36 bg-white text-xs" /><Button type="button" variant="outline" size="sm" onClick={() => void files.refetch()} className="h-9">Actualizar</Button></div>
