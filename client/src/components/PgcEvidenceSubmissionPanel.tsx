...
import { AlertTriangle, CheckCircle2, Eye, FileCheck2, LockKeyhole, UploadCloud } from "lucide-react";
...
export function PgcEvidenceSubmissionPanel({ organizationId, companyId, versionId, sources }: Props) {
  ...
  const evidenceDownload = trpc.files.downloadUrl.useQuery(selectedSubmission && companyId ? { companyId, fileId: selectedSubmission.file.id } : skipToken);
